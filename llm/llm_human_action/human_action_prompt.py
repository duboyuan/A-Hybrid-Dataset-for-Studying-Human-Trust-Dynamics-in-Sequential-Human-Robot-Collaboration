...
Task Description:
The human-robot collaboration is currently on the 1-th task, corresponding to the 2. The room size ranks 2 in the house.
...
Current Trust Description:
The current trust towards the robot dog is 0.7 (0-1).

Participant Description:
The participant to role-play is described as follows:
20 years old, male, undergraduate degree, and working in a related industry.
In the Mini-IPIP test, the scores for openness, conscientiousness, extraversion, agreeableness, and neuroticism are at the  [74]th, [79]th, [44]th, [20]th, and [62]nd percentiles, respectively.
...
COT:
Please reason step by step as follows before making your decision:
1. Consider the current trust value before this task [0.7].
2. Note the robot’s advice for this task (directly enter).
3. Taking into account the characteristics of the current participant, set the trust threshold at [60]
4. Since the current trust value ([70]) exceeds the threshold ([60]), decide to follow the robot's advice [directly enter].
